    ...
    >
      ...
      <form @submit.prevent="submitOrder">
        ...
        <div class="mb-3">
          ...
          <input v-model="orderData.pickUpPhoneNumber" type="tel" class="form-control" id="pickupPhoneNumber" />
        </div>

        <div class="mb-4">
          <label for="pickupEmail" class="form-label">Email</label>
          <input v-model="orderData.pickUpEmail" type="email" class="form-control" id="pickupEmail" />
        </div>
        ...
        <div class="d-flex justify-content-end gap-2">
          <button @click="closeModal" type="button" class="btn btn-outline-secondary px-4">Cancel</button>
          <button type="submit" class="btn btn-success px-4" :disabled="isSubmitting || !cartStore.cartItems ||cartStore.cartItems.length===0">
...
const submitOrder = async () => {
  try {
    ...
    errorList.push('An error occurred while submitting your order. Please try again.', error.message);
  }
  finally {
    isSubmitting.value = false;
  }
