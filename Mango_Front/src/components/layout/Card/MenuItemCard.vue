    ...
    <div class="col-12 col-sm-6 col-lg-4">
        <div class="card h-100 border-0 shadow-sm rounded-4">
            <div class="position-relative">
                <img :src="CONFIG_IMAGE_URL + menuItem.image" class="card-img-top rounded-top-4 w-100 object-fit-cover"
                    style="height: 200px" />
                <!-- View Details Button -->
                <div class="position-absolute top-0 end-0 m-2">
                    <button
                        class="btn btn-light btn-sm rounded shadow-sm d-flex align-items-center gap-2 backdrop-blur-sm bg-opacity-75"
                        title="View Details" style="backdrop-filter: blur(4px)" @click="emit('showDetails', menuItem)">
            ...
            <div class="card-body p-3 p-sm-4">
                ...
                <div class="d-flex align-items-center justify-content-between">
                    <button class="btn btn-success w-100 rounded py-2" @click="addToCart" :disabled="isProcessing" v-if="!isInCart">
                    ...
                    <div class="input-group input-group-sm w-100"  v-else>
                        ...
                        <input type="text" class="form-control text-center px-2" readonly style="max-width: 50px" :value="itemQuantaty" />
...
const increaseQuantity = () => {
    cartStore.updateQuantity(props.menuItem.id, itemQuantaty.value + 1);
};
const decreaseQuantity = () => {
    if (itemQuantaty.value > 1) {
        cartStore.updateQuantity(props.menuItem.id, itemQuantaty.value - 1);
    } else {
